...
perc_under_buy     = 0.76
perc_under_trl_buy = 1.02
perc_under_target  = 1.12
perc_under_trl_tar = 0.98
...
def loading_settings2():
    global ma_selected, qty_syb_2, perc_under_buy, perc_under_trl_buy, perc_under_target, perc_under_trl_tar

    ma_selected =  session.query(Fxt_Settings).filter(Fxt_Settings.name == 'ma_selected').first().value
    qty_syb_2 =  float(session.query(Fxt_Settings).filter(Fxt_Settings.name == 'qty_syb_2').first().value)

    perc_under_buy =  float(session.query(Fxt_Settings).filter(Fxt_Settings.name == 'perc_under_buy').first().value)
    perc_under_buy = (100 - perc_under_buy) / 100

    perc_under_trl_buy = float(session.query(Fxt_Settings).filter(Fxt_Settings.name == 'perc_under_trl_buy').first().value)
    perc_under_trl_buy = (100 + perc_under_trl_buy) / 100

    perc_under_target =  float(session.query(Fxt_Settings).filter(Fxt_Settings.name == 'perc_under_target').first().value)
    perc_under_target =  (100 + perc_under_target) / 100

    perc_under_trl_tar = float(session.query(Fxt_Settings).filter(Fxt_Settings.name == 'perc_under_trl_tar').first().value)
    perc_under_trl_tar = (100 - perc_under_trl_tar) / 100
...
def func_buy_qty():
    global cur_buy_qty
    cur_buy_qty = (qty_syb_2 / cur_close) * 0.99
